...
7. If a whole section's data is missing (e.g. no `statsCache` → no token totals, no `transcripts` → no supporting-cast slide), delete that `<section>` entirely rather than faking numbers. The deck degrades gracefully — JS derives slides and dots dynamically. If you delete the persona section, still fill `HOUR_DATA` and `HOT_HOURS` — set both to `[]`.
