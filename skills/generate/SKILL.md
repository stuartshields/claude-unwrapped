...
- `references/share-file.md` — Step 5: the share-file privacy review, the exact v2 JSON schema, length caps, and the third-person rewrite. Consulted only on share runs.
- `references/update-slide.md` — the single-slide update mode. Consulted only when refreshing one slide of an existing deck.
