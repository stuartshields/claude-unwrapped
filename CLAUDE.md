...
- `skills/generate/scripts/analyze.py` — stdlib-only aggregator. Reads `stats-cache.json`, `history.jsonl`, `projects/*/*.jsonl`. Honors `CLAUDE_CONFIG_DIR`, accepts a dir as argv[1], emits one JSON to stdout. Every section nullable — never crash on missing files.
...
- Test commands: `python3 skills/generate/scripts/analyze.py ~/.claude | python3 -m json.tool > /dev/null` (analyzer), HTML tag-balance + `node --check` on the extracted script (template).
